 ...
 * <pre>{@code
 * // Simple translation
 * Component msg = Colors.translate("&aHello <bold>World</bold>!");
 *
 * // With PlaceholderAPI
 * Component msg = Colors.translate("Welcome %player_name%!", player);
 *
 * // Sending to player
 * Colors.sendMessage(player, "&c&lWarning: <hover:show_text:'Details'>hover</hover>");
 * }</pre>
 *
 * <h2>Design Notes</h2>
 * <ul>
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c == '{' && i + 8 < len && chars[i + 1] == '#' && isHexSequence(chars, i + 2, 6)) {
                ...
                if (afterHex + 1 < len && chars[afterHex] == '>' && chars[afterHex + 1] == '}') {
                    if (gradOpen) {
                        out.append("{#").append(gradColors.get(0)).append('>').append(gradBuf);
                    }
                    gradOpen = true;
